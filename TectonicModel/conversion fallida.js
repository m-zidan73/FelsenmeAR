...
export class FelsenmeARModel {
  ...
  get contraction() { return this._contraction; }
  ...
  _applyDeformation(time) {
    ...
    if (this._capaA && this._origA && this._tA) {
      ...
      if (!this._g2bMap) this._g2bMap = this._buildGlbToBlenderMap();
      if (!this._ribVertSet || !this._edgePairs) {
        const edges = [[13,0],[14,7],[16,1],[18,2],[20,5],[22,11],[23,10],[8,12],[3,15],[6,17],[4,19],[9,21],[26,25],[24,27],[30,29],[28,31],[34,33],[32,35]];
        const ribBlender = new Set(edges.flat());
        this._ribVertSet = new Set();
        this._edgePairs = [];
        for (const [a, b] of edges) {
          const ga = [], gb = [];
          for (let gi = 0; gi < this._g2bMap.length; gi++) {
            if (this._g2bMap[gi] === a) ga.push(gi);
            if (this._g2bMap[gi] === b) gb.push(gi);
          }
          if (ga.length && gb.length) {
            this._edgePairs.push({ a: ga, b: gb });
            ga.forEach(i => this._ribVertSet.add(i));
            gb.forEach(i => this._ribVertSet.add(i));
          }
        }
      }
      const cRatio = this._contraction * 4;
      for (const ep of this._edgePairs) {
        let zA = 0, zB = 0;
        for (const i of ep.a) zA += arrA[i * 3 + 2];
        for (const i of ep.b) zB += arrA[i * 3 + 2];
        zA /= ep.a.length;
        zB /= ep.b.length;
        const midZ = (zA + zB) / 2;
        for (const i of ep.a) arrA[i * 3 + 2] += (midZ - zA) * cRatio;
        for (const i of ep.b) arrA[i * 3 + 2] += (midZ - zB) * cRatio;
      }
      for (let i = 0, vi = 0; i < arrA.length; i += 3, vi++) {
        if (!this._ribVertSet.has(vi))
          arrA[i + 2] += dz;
  ...
  _buildBlenderToGlbMap() {
    const bVerts = [
      [0.278708,-0.987369,1.659364],[-0.284715,-0.987369,1.659364],[-0.284715,0.278911,1.448212],
      [0.278708,0.278911,1.448212],[0.278708,1.04604,0.751033],[-0.284715,1.04604,0.751033],
      [-0.284715,0.801552,1.039746],[0.278708,0.801552,1.039746],[-0.284715,-0.443446,1.610961],
      [0.278708,-0.443446,1.610961],[-0.284715,-2.297578,1.659364],[0.278708,-2.297578,1.659364],
      [-0.284715,-0.603386,1.454569],[0.278708,-0.987369,1.492162],[0.278708,0.501215,0.95485],
      [0.278708,0.081237,1.277069],[-0.284715,-0.987369,1.492162],[-0.284715,0.501215,0.95485],
      [-0.284715,0.081237,1.277069],[0.278708,0.730798,0.751033],[-0.284715,0.730798,0.751033],
      [0.278708,-0.603386,1.454569],[0.278708,-2.297578,1.492162],[-0.284715,-2.297578,1.492162],
      [0.278708,1.093814,0.702793],[-0.230652,1.093814,0.702793],[-0.230652,0.767099,0.702793],
      [0.278708,0.767099,0.702793],[0.278708,-1.642474,1.659364],[-0.284715,-1.642474,1.659364],
      [-0.284715,-1.642474,1.492162],[0.278708,-1.642474,1.492162],[0.278708,-1.970026,1.659364],
      [-0.284715,-1.970026,1.659364],[-0.284715,-1.970026,1.492162],[0.278708,-1.970026,1.492162],
    ];
    const arr = this._origA;
    const eps = 0.01;
    const map = new Array(bVerts.length).fill(-1);
    const count = arr.length / 3;
    for (let bi = 0; bi < bVerts.length; bi++) {
      const bx = bVerts[bi][0], by = bVerts[bi][1], bz = bVerts[bi][2];
      let best = -1, bestD = Infinity;
      for (let gi = 0; gi < count; gi++) {
        const dx = arr[gi*3] - bx;
        const dy = arr[gi*3+1] - bz;
        const dz = arr[gi*3+2] + by; // Blender y -> -GLB z
        const d = dx*dx + dy*dy + dz*dz;
        if (d < bestD) { bestD = d; best = gi; }
      }
      if (bestD < eps) map[bi] = best;
      else console.warn("b2g unmapped", bi, "best dist", Math.sqrt(bestD));
    }
    console.log("b2gMap:", JSON.stringify(Array.from(map)));
    return map;
  }

  _buildGlbToBlenderMap() {
    const bVerts = [
      [0.278708,-0.987369,1.659364],[-0.284715,-0.987369,1.659364],[-0.284715,0.278911,1.448212],
      [0.278708,0.278911,1.448212],[0.278708,1.04604,0.751033],[-0.284715,1.04604,0.751033],
      [-0.284715,0.801552,1.039746],[0.278708,0.801552,1.039746],[-0.284715,-0.443446,1.610961],
    ...
    const eps = 0.01;
    const count = arr.length / 3;
    const g2b = new Array(count).fill(-1);
    for (let gi = 0; gi < count; gi++) {
      for (let bi = 0; bi < bVerts.length; bi++) {
        const dx = arr[gi*3] - bVerts[bi][0];
        const dy = arr[gi*3+1] - bVerts[bi][2];
        const dz = arr[gi*3+2] + bVerts[bi][1];
        if (dx*dx + dy*dy + dz*dz < eps) { g2b[gi] = bi; break; }
      }
    }
    return g2b;
  }

  _showDebugContraction() {
    if (!this._capaA) return;
    if (!this._b2gMap) this._b2gMap = this._buildBlenderToGlbMap();
    const map = this._b2gMap;
    const arr = this._origA;
    const edges = [[13,0],[14,7],[16,1],[18,2],[20,5],[22,11],[23,10],[8,12],[3,15],[6,17],[4,19],[9,21],[26,25],[24,27],[30,29],[28,31],[34,33],[32,35]];
    const lineData = [];
    for (const [a, b] of edges) {
      const ga = map[a], gb = map[b];
      if (ga < 0 || gb < 0) continue;
      lineData.push(arr[ga*3], arr[ga*3+1], arr[ga*3+2], arr[gb*3], arr[gb*3+1], arr[gb*3+2]);
    }
    const g = new THREE.BufferGeometry();
    g.setAttribute("position", new THREE.Float32BufferAttribute(lineData, 3));
    this.scene.add(new THREE.LineSegments(g, new THREE.LineBasicMaterial({ color: 0x00ff00 })));
  }
